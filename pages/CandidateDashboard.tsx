  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col items-center">
          <div className="h-10 w-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-3"><i className="fas fa-clipboard-list"></i></div>
          <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Total Interviews</h3>
          <p className="text-3xl font-extrabold text-gray-800 mt-1">{stats.total}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col items-center">
          <div className="h-10 w-10 rounded-full bg-yellow-50 text-yellow-600 flex items-center justify-center mb-3"><i className="fas fa-star"></i></div>
          <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Shortlisted</h3>
          <p className="text-3xl font-extrabold text-gray-800 mt-1">{stats.shortlisted}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col items-center">
          <div className="h-10 w-10 rounded-full bg-green-50 text-green-600 flex items-center justify-center mb-3"><i className="fas fa-check-circle"></i></div>
          <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Hired</h3>
          <p className="text-3xl font-extrabold text-gray-800 mt-1">{stats.hired}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col items-center">
          <div className="h-10 w-10 rounded-full bg-red-50 text-red-600 flex items-center justify-center mb-3"><i className="fas fa-times-circle"></i></div>
          <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Rejected</h3>
          <p className="text-3xl font-extrabold text-gray-800 mt-1">{stats.rejected}</p>
        </div>
      </div>
      )}
        ...
        <div className="mb-10">
          ...
          {displayedMatches.length === 0 ? (
            ...
            {displayedMatches.map(job => {
              ...
              return (
                <div key={job.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-l-4 border-l-yellow-400">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 flex justify-end gap-2">
                    <button 
                      ...
                      className="bg-white border border-gray-200 text-gray-600 hover:text-primary hover:border-primary px-3 py-2 rounded text-sm font-medium transition-colors"
                    >
                      View Details
                    </button>
      ...
      {!onlyBestMatches && (
      <>
      <h2 className="text-2xl font-bold text-gray-800 mb-6 px-1">Available Opportunities</h2>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="relative">
             <i className="far fa-calendar-alt absolute left-3 top-3 text-gray-400"></i>
             <select 
               className="w-full pl-10 pr-4 py-2 border rounded-md focus:ring-primary focus:border-primary appearance-none bg-white"
               value={sortOrder}
      ...
      {filteredJobs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredJobs.map(job => {
            ...
            return (
              <div key={job.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold text-primary">{job.title}</h3>
                  {job.interviewPermission === 'request' && (
                    <span className="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded border border-blue-100">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-100 flex justify-between items-start sticky top-0 bg-white z-10">
              <div>
                ...
                <p className="text-gray-600 font-medium"><i className="fas fa-building mr-2"></i>{selectedJob.companyName}</p>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-xl">
                      ...
                      <p className="font-medium text-gray-800">{(selectedJob as any).location || 'Remote / Not Specified'}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-xl">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Salary Range</p>
                      <p className="font-medium text-gray-800">{(selectedJob as any).salaryRange || 'Not Disclosed'}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-xl">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Job Type</p>
                      <p className="font-medium text-gray-800">{(selectedJob as any).jobType || 'Full-time'}</p>
                  </div>
                   <div className="bg-gray-50 p-4 rounded-xl">
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Deadline</p>
                      <p className="font-medium text-gray-800">{selectedJob.applyDeadline?.toDate ? selectedJob.applyDeadline.toDate().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Open until filled'}</p>
                  </div>
              </div>
              ...
              <div>
                  <h3 className="text-lg font-bold text-gray-900 mb-2">Required Qualifications</h3>
